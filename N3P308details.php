...
function generate_ratings($rating) {
    $movie_rating = '';
    for ($i = 0; $i < $rating; $i++) {
        $movie_rating .= '<img width="12" height="12" src="./star.png" alt="star"/>';
    }
    return $movie_rating;
}
...
//º take in the id of a lead actor and return his/her full name
function get_leadactor($leadactor_id) {

    global $db;

    $query = 'SELECT people_fullname FROM people WHERE people_id = ' . $_GET['movie_id'];
    $result = mysqli_query($db, $query) or die(mysqli_error($db));
...
$query = 'SELECT review_movie_id, review_date, reviewer_name, review_comment, review_rating FROM reviews WHERE review_movie_id='
. $_GET['movie_id'] . ' ORDER BY review_date ' . $_GET['order'];
...
if ($state == "ASC")
{
	echo '<a href="./N3P308details.php?movie_id=6&order=DESC">Date</a>';
}

if ($state == "DESC")
{
	echo '<a href="./N3P308details.php?movie_id=6&order=ASC">Date</a>';
}
